...
const RECIPES: Recipe[] = [
  ...
  },
  {
    ...
    steps: ["Jajka ugotuj na miękko (6 min).", "Awokado rozgnieć z cytryną i solą na pastę.", "Rozsmaruj na pieczywie, połóż rozkrojone jajka.", "Posyp świeżo zmielonym pieprzem."]
  },
  {
...
export default function Home() {
  ...
  const handleAddFood = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!foodInput || isLoading) return;
    setIsLoading(true);
    try {
      const lowerInput = foodInput.trim().toLowerCase();
      const alreadyExists = meals.find(m => m.name.toLowerCase() === lowerInput);
      let nutrition = alreadyExists ? { kcal: alreadyExists.kcal, protein: alreadyExists.protein, carbs: alreadyExists.carbs, fat: alreadyExists.fat } : await analyzeFood(foodInput);
      const { data, error } = await supabase.from('meals').insert([{ name: foodInput.trim(), kcal: Math.round(nutrition.kcal || 0), protein: Math.round(nutrition.protein || 0), carbs: Math.round(nutrition.carbs || 0), fat: Math.round(nutrition.fat || 0) }]).select();
      if (error) throw error;
      if (data) { setMeals([data[0], ...meals]); setFoodInput(''); }
    } catch (err) { alert("Błąd AI lub limit. Spróbuj za chwilę!"); } finally { setIsLoading(false); }
  };
